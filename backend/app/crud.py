...
async def create_bot(
    db: AsyncSession, data: bot_schema.BotCreate, encrypted_key: str | None
) -> Bot:
    bot = Bot(
        name=data.name,
        area=data.area,
...
async def get_bot_by_id(db: AsyncSession, bot_id: int) -> Bot | None:
    result = await db.execute(select(Bot).where(Bot.id == bot_id))
    return result.scalar_one_or_none()


async def get_bot_by_name(db: AsyncSession, name: str) -> Bot | None:
